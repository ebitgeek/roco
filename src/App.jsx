...
function App() {
    ...
    useEffect(() => {
        fetch(`/roco/json/merchant-current.json?v=${Date.now()}`)
            .then((res) => res.json())
            .then(setData)
            .catch(console.error);

        fetch(`/roco/json/merchant-history.json?v=${Date.now()}`)
            .then((res) => res.json())
            .then(setHistory)
            .catch(console.error);
    }, []);
                        ...
                        <div className="flex flex-col gap-4 mb-6">
                            {
                                history.rounds.map((round) => (
                                    <Collapse question={`${round.date} ${round.slot}`} defaultExpanded
                                              answer={
